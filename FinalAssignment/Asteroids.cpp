...
void Asteroids::shoot(float startX, float startY, float targetX, float targetY)
{
    ...
    float range = 1000; // bullet cant go past the player past 1000 pixels
    ...
    float angle = ((atan2(direction.y, direction.x) * 180) / 3.141) + 90; //sets the direction in which the asteroid will be facing
...
void Asteroids::update(float elapsedTime)
{
    ...
    // Has the astroid gone out of range?
